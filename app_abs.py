...
st.set_page_config(
    page_title="Predicción PETG FDM",
    page_icon="🧠",
    layout="wide"
)

st.title("🧠 Predicción directa e inversa en piezas PETG impresas por FDM")
st.write(
    "Modelo basado en redes neuronales MLP para predecir la resistencia a flexión "
    "y el costo total de fabricación."
)

st.warning(
    "El modelo de flexión presenta capacidad predictiva moderada. "
...
costo_min_real = y_costo.min()
costo_max_real = y_costo.max()
...
def predecir_directo(temperatura, densidad, orientacion):
    ...
    flexion = modelo_flexion.predict(entrada)[0]
    costo = modelo_costo.predict(entrada)[0]

    return flexion, costo


def penalizacion_costo(costo_predicho):
    if costo_predicho < costo_min_real:
        return 1e6 + abs(costo_min_real - costo_predicho) * 1e6

    if costo_predicho > costo_max_real:
        return 1e6 + abs(costo_predicho - costo_max_real) * 1e6
...
def prediccion_inversa_resistencia_min_costo(flexion_objetivo):
    def funcion_objetivo(parametros):
        ...
        penalizacion = penalizacion_costo(costo_predicho)
    ...
    temperatura, densidad, orientacion = resultado.x
    ...
    return temperatura, densidad, orientacion, flexion_predicha, costo_predicho
...
pred_costo = modelo_costo.predict(X)
...
with tab1:
    ...
    st.write("Ingrese los parámetros de impresión dentro del dominio experimental.")
    ...
    if st.button("Predecir", key="directa"):
        flexion_predicha, costo_predicho = predecir_directo(
            temperatura,
            densidad,
            orientacion
...
with tab2:
    ...
    if st.button("Buscar parámetros", key="inversa1"):
        ...
        col1, col2, col3 = st.columns(3)
        col1.metric("Temperatura", f"{T:.2f} °C")
        col2.metric("Densidad", f"{D:.2f} %")
        col3.metric("Orientación", f"{O:.2f} °")

        col4, col5 = st.columns(2)
        col4.metric("Flexión predicha", f"{flexion_predicha:.2f} MPa")
        col5.metric("Costo predicho", f"{costo_predicho:.3f} USD")

        st.info(
            "La solución busca aproximarse a la resistencia objetivo manteniendo bajo el costo predicho."
        )
    ...
    with col2:
        costo_objetivo = st.number_input(
            "Costo objetivo (USD)",
            min_value=float(costo_min_real),
            max_value=float(costo_max_real),
            value=0.350,
            step=0.01
        )

    if st.button("Buscar solución doble objetivo", key="inversa2"):
...
with tab3:
    ...
    with col1:
        st.subheader("Modelo de flexión")
    ...
    with col2:
        st.subheader("Modelo de costo")
        st.metric("R² ajuste", f"{r2_costo:.3f}")
        st.metric("RMSE", f"{rmse_costo:.4f} USD")
        st.metric("MAE", f"{mae_costo:.4f} USD")
        st.caption("Arquitectura MLP: 3 entradas – 5 y 5 neuronas ocultas – 1 salida")

    st.subheader("Experimental vs predicho")
    ...
    ax1.set_title("Flexión: experimental vs predicho")
    ax1.grid(alpha=0.3)
    st.pyplot(fig1)
    ...
    ax2.set_title("Costo: experimental vs predicho")
    ax2.grid(alpha=0.3)
    st.pyplot(fig2)

    st.subheader("Relación resistencia-costo")
...
with tab4:
    ...
    orientacion_fija = st.slider(
        "Orientación fija para generar la superficie (°)",
        min_value=0,
        max_value=90,
        value=45,
        step=5
    ...
    for i in range(T_grid.shape[0]):
        for j in range(T_grid.shape[1]):
            ...
            Z_flexion[i, j] = flexion_predicha
            Z_costo[i, j] = costo_predicho

    st.subheader("Superficie de respuesta para flexión")

    fig4, ax4 = plt.subplots(figsize=(7, 5))
    contour1 = ax4.contourf(
        D_grid,
        T_grid,
        Z_flexion,
        levels=20
    ...
    st.subheader("Superficie de respuesta para costo")

    fig5, ax5 = plt.subplots(figsize=(7, 5))
    contour2 = ax5.contourf(
        D_grid,
        T_grid,
        Z_costo,
        levels=20
    ...
    st.info(
        "Las superficies se generan manteniendo fija la orientación seleccionada y variando temperatura y densidad."
    )